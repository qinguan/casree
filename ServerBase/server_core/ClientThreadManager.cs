    ...
    class ClientThreadManager
    {
        ...
        public static List<string> GetActiveClients()
        {
            ...
            lock (clientList)
            {
                ...
                {
                    activeList.Add(pair.Value.projectid);
                }
            }
            return activeList;
        }
        public static ClientInfo GetClient(string projectID)
        {
            lock (clientList)
            {
                foreach (KeyValuePair<string, ClientInfo> pair in clientList)
                {
                    if(pair.Value.projectid.Equals(projectID))
                        return pair.Value;
                }
            }
            return null;
        ...
        {   //关掉指定ip&port客户端连接(考虑到同一IP的多个客户端连接，需要端口来区别)
            ...
            lock (clientList)
            {
                foreach (KeyValuePair<string, ClientInfo> s in clientList)
                {
                    Console.WriteLine(s.ToString()+"待删除");
                    Console.WriteLine("s.key的值是"+s.Key);
                    Console.WriteLine("客户端工程ID为"+s.Value.projectid);
                    Console.WriteLine("ip值是"+ip);
                    if (s.Key.CompareTo(ip) == 0)
                    {
                        ...
                        //thread.Abort()引发 System.Threading.ThreadAbortException，以开始终止此线程的过程。调用此方法通常会终止线程。
        ...
        { //客户端超时判断
            // foreach (KeyValuePair<string, ClientInfo> s in clientList)//遍历时不能直接更改集合2012.10.12
            for (int i = 0; i < clientList.Count; i++)
            {
                KeyValuePair<string, ClientInfo> temppair = clientList.ElementAt<KeyValuePair<string, ClientInfo>>(i);
                if (DateTime.Now.Subtract(temppair.Value.lastAliveTime).TotalSeconds > 500) //测试给出了50s
                {
